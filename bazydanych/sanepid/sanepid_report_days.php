...
<body>
    <div id="container">
        ...
        if ($user->_is_logged_as('sanepid')) {
            ...
            $query = "SELECT * FROM projekt.widok_raportow WHERE wojewodztwo_nazwa = (SELECT w.wojewodztwo_nazwa FROM projekt.sanepid s, projekt.wojewodztwo w WHERE s.wojewodztwo_id = w.wojewodztwo_id AND s.sanepid_id = {$_SESSION['sanepid']})";
            if (isset($_POST['submit'])) 
            {
                if (!empty($_POST['period'])) 
                {
                    if($_POST['period'] != "all")
                    {
                        $query .= " AND zakazony_data_wpisu between (NOW() - interval '{$_POST['period']}') AND NOW()";
                    }
                }
